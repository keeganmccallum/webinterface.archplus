...
angular.module('app')
.controller('HeaderNavController', ['$scope', '$location', '$filter', '$http',
  function ($scope, $location, $filter, $http) {
    $scope.showWizard = false;
    $scope.query = '';
    $scope.isOpen = false;
    $scope.playUrlQuery = '';
    $scope.isPlayOpen = false;
    $scope.medias = [{
      hash: '/movies/recents',
      label: 'Movies',
      matchRegExp : /movie.*$/
    ...
    $scope.openPlay =  function () {
      $scope.isPlayOpen = !$scope.isPlayOpen;
      document.querySelector('header nav .playUrl input').focus();
    }
    ...
    $scope.playUrl = function () {
      var RpcUrl = `/jsonrpc?request={"jsonrpc":"2.0",%20"id":1,%20"method":%20"Player.Open",%20"params":{"item":{"file":"${encodeURI($scope.playUrlQuery)}"}}}`;
      $http.get(RpcUrl).then(function (resp) {
        if (resp.data.error) alert('Unable to play video: '+resp.data.error.message);
      });
      $scope.isPlayOpen = false;
      $scope.playUrlQuery = '';
    };
...
angular.module("modules/common/navigation.tpl.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/common/navigation.tpl.html",
    "<nav class=\"row\">\n" +
    "    <form class=\"clearfix search playUrl\" ng-submit=\"playUrl()\" ng-class=\"{open : isPlayOpen, span3: isPlayOpen, spanButton: !isPlayOpen}\">\n" +
    "        <div class=\"btn\" ng-click=\"openPlay()\">\n" +
    "            <i class=\"fa fa-play\"></i>\n" +
    "        </div>\n" +
    "        <input type=\"text\" placeholder=\"Play URL\" ng-model=\"playUrlQuery\"/>\n" +
    "    </form>\n" +
